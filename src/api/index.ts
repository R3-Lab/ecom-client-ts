...
export const API_BASE: string = process.env.SERVER_URL || (isProd ? 'https://backend.bitnarts.com' : 'http://localhost:5800');
export const SITE_ID: string = process.env.SITE_ID || 'ebc1e210-d8d7-46be-ba98-349b59b4e3dd';
...
export const fetcher = async  <T,>(path: string, options: RequestInit = {}): Promise<FetcherResponse<T>> => {
...
export const setSiteIdSession = async (site_id?: string, current_path?: string) => {
    if (!site_id) {
